...
def main():
    ...
    plt.plot(predictions[:200], label='GRU Predictions')
    plt.title('GRU Model (PyTorch) - Electricity Consumption Prediction (Sample)')
    plt.xlabel('Time (half-hour intervals)')
    plt.ylabel('KWH')
    plt.legend()
    plt.savefig('gru_predictions.png')
    print("Plot saved to gru_predictions.png")
